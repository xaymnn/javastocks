...
public class ModifArticleController {
    ...
    protected void onModifierButtonClick() {
        ...
        try {
            ...
            updateArticleDetails(selectedArticleID, libelle, categorie, taille, couleur, volume, poids, quantite);
        ...
        if (connection != null) {
            try {
                ...
                if (resultSet.next()) {
                    ...
                    volumeTextField.setText(resultSet.getString("volume"));
    ...
    private void updateArticleDetails(Integer articleID, String libelle, String categorie, String taille, String couleur, String volume, String poids, int quantite) {
        // Établir la connexion à la base de données
        DatabaseConnection databaseConnection = new DatabaseConnection();
        Connection connection = databaseConnection.getConnection();
        ...
        if (connection != null) {
            try {
                ...
                String sql = "UPDATE Article SET libelle = ?, categorie = ?, taille = ?, couleur = ?, volume = ?, poids = ?, quantite = ? WHERE article_id = ?";
                PreparedStatement preparedStatement = connection.prepareStatement(sql);
                ...
                preparedStatement.setString(6, poids);
                preparedStatement.setInt(7, quantite);
